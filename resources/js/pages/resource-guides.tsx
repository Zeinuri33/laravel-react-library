...
export default function ResourceGuidesPage() {
    ...
    const resourceGuides = useMemo(
        () => (page.props.resourceGuides as ResourceGuide[]) || [],
        [page.props.resourceGuides]
    )
    ...
    const filteredGuides = useMemo(() => {
        ...
        return resourceGuides.filter((item) => {
            const prodi = item.prodi?.prodi?.toLowerCase() || ""
            const fakultas = item.prodi?.fakultas?.fakultas?.toLowerCase() || ""
            const kode = item.prodi?.kode?.toLowerCase() || ""

            return (
                prodi.includes(query) ||
                fakultas.includes(query) ||
                kode.includes(query)
            )
        })

    }, [resourceGuides, search])

    const groupedGuides = useMemo(() => {

        const sortedGuides = [...filteredGuides].sort(
            (a, b) =>
                (a.prodi?.fakultas?.id || 0) -
                (b.prodi?.fakultas?.id || 0)
        )

        const grouped = sortedGuides.reduce(
            (
                acc: Record<string, ResourceGuide[]>,
                item
            ) => {

                const fakultas =
                    item.prodi?.fakultas?.fakultas || "Lainnya"

                if (!acc[fakultas]) {
                    acc[fakultas] = []
                }

                acc[fakultas].push(item)
        ...
        // SORT PRODI DI DALAM SETIAP FAKULTAS
        for (const key of Object.keys(grouped)) {
            grouped[key].sort((a, b) =>
                (a.prodi?.prodi || "").localeCompare(b.prodi?.prodi || "", "id")
            )
        }
    ...
    const activeGuide =
        (page.props.activeGuide as ResourceGuide) || null
    ...
    useEffect(() => {
        ...
        if (!articleRef.current || !activeGuide) {
            return
        }
                                        ...
                                        return (
                                            <div key={fakultas}>
                                                ...
                                                >
                                                    ...
                                                    {guides.map((guide: ResourceGuide) => {
                                                        ...
                                                        return (
                                                            <Link
                                                                key={guide.id}
                                                                href={`/resource-guides/${guide.slug}`}
                                                                onClick={() => {
                                                                    setMobileOpen(false)
                                                                }}
                                                                className={`
                                    ...
                                    <div className="text-center">
                                        ...
                                        <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                                            {search
                                                ...
                                                : "Belum ada resource guide tersedia."}
                                        </p>
                                    </div>
                                )}
                            </nav>
                                    ...
                                    {activeGuide ? (
                                        <>
                                            ...
                                            >
                                                ...
                                                {activeGuide.prodi?.fakultas?.fakultas}
                                            </div>
                                            ...
                                            <h1
                                                ...
                                                `}
                                            >
                                                {activeGuide.prodi?.prodi}
                                            </h1>

                                            {activeGuide.prodi?.kode && (
                                        ...
                                        >
                                            <div className="text-center">
                                                ...
                                                <BookMarked className="mx-auto h-10 w-10 text-gray-300 dark:text-gray-600" />

                                                <h2 className="mt-3 text-2xl font-bold">
                                                    Resource Guide kosong
                                                </h2>
